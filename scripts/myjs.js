...
$(".item").on('click',function(){
	...
	if(data_label[label].link){
		var pre = "<div id='pop_data' class='container'><div class='pop_video col-xs-6'>"+data_label[label].link+"</div><div class='col-xs-6'>"+"<div class='pop_content'>Name : "+data_label[label].name+"</div><a href='"+data_label[label].url+"' target='_blank'><div class='pop_content'>"+data_label[label].url+"</div></a><div class='pop_info'>"+data_label[label].info+"</div><div id='close'><i class='icon-remove-sign icon-2x'></i></div></div>"
	}else{
		var pre = "<div id='pop_data' class='container'><div class='col-xs-6 col-xs-offset-4'>"+"<div class='pop_content'>Name : "+data_label[label].name+"</div><a href='"+data_label[label].url+"' target='_blank'><div class='pop_content'>"+data_label[label].url+"</div></a><div class='pop_info'>"+data_label[label].info+"</div><div id='close'><i class='icon-remove-sign icon-2x'></i></div></div>"
	}
	$("html").prepend(pre);
	$(".all_mask").slideDown();
